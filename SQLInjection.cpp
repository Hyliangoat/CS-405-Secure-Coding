...
#include <tuple>
#include <vector>
//Added regex to the includes to use regex for SQL injection detection
#include <regex>
...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  /* Detecting an SQL injection with the following format
  * OR literal = same_literal
  * The literal can be a numeric value like 1 2 -5, 3.1415, etc
  * Or a quoted string like 'hi' or 'hackerman' or '1=1' etc
  *The backreference \1 will require the value on the right side of the equals sign to be the same as the value on the left side of the equals sign
  */

  const std::regex injection_pattern(R"(\b[Oo][Rr]\s+((?:'(?:''|[^'])*')|(?:[+-]?(?:(?:\d+(?:\.\d*)?)|(?:\.\d+))(?:[eE][+-]?\d+)?))\s*=\s*\1\s*;?)");


  //Now we search the complete SQL query for the pattern of an SQL injection, or anything suspicious that looks like an SQL injection
  //Show the sql being used, and confirm blocking the query from being executed if an SQL injection is detected
  if (std::regex_search(sql, injection_pattern))
  {
      std::cout << std::endl;
	  std::cout << "Possible SQL injection detected in the query: " << sql << std::endl;
	  std::cout << "Blocked sql query from being executed." << std::endl;
      return false;
  }
